...
ip netns exec ns1 ip addr add dev eth0 10.0.1.102/24
ip netns exec ns3 ip addr add dev eth0 10.0.3.102/24
...
ovs-vsctl add-port br-tun vxlan103 -- set interface vxlan103 type=vxlan options:in_key=flow options:local_ip=192.168.57.102 options:out_key=flow  options:remote_ip=192.168.57.103
...
ip addr add dev int-dvr1 10.0.1.2/24
ip addr add dev int-dvr3 10.0.3.2/24
...
# Make sure that patch-tun: port 3, vxlan101: port 1 and vxlan103: port 2
...
ovs-ofctl add-flow br-tun "table=21,priority=1,dl_vlan=1,actions=strip_vlan,set_tunnel:0x1,output:1,output:2"
ovs-ofctl add-flow br-tun "table=21,priority=1,dl_vlan=3,actions=strip_vlan,set_tunnel:0x3,output:1,output:2"
