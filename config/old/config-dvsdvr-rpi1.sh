...
if [ "$1" != "reboot" ]; then
	...
	ovs-vsctl add-port br-tun vxlan101 -- set interface vxlan101 type=vxlan options:in_key=flow options:local_ip=192.168.1.104 options:out_key=flow  options:remote_ip=192.168.1.101
	ovs-vsctl add-port br-tun vxlan102 -- set interface vxlan102 type=vxlan options:in_key=flow options:local_ip=192.168.1.104 options:out_key=flow  options:remote_ip=192.168.1.102
	ovs-vsctl add-port br-tun vxlan103 -- set interface vxlan103 type=vxlan options:in_key=flow options:local_ip=192.168.1.104 options:out_key=flow  options:remote_ip=192.168.1.103
...
# VLAN-related config.
if [ "$1" == "reboot" ]; then
	python ./config-vlan.py $model 1 10.0.1.1/24 10.0.1.104/24 reboot
	python ./config-vlan.py $model 3 10.0.3.1/24 10.0.3.104/24 reboot 
else
	python ./config-vlan.py $model 1 10.0.1.1/24 10.0.1.104/24
	python ./config-vlan.py $model 3 10.0.3.1/24 10.0.3.104/24
fi
